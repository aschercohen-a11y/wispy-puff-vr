...
- 🍄 **Champignons = obstacles** : `mushroomObstacles[]` (x,z,r,top calculés depuis la bbox). Collision push-out tant que `hero.y < top` → il faut **sauter par-dessus**. Si au sol + fonce dedans → **chute** (`heroFallT=1.2s`, immobilisé, anim `falling_down`, `HERO_RADIUS=0.28`). Tenir le stick contre le champignon = re-chute (force à sauter).
