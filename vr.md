...
- ✅ **VRAI PERSO Meshy intégré** : `models/character.glb` (bandicoot style Crash, Meshy image-to-3D + rig + anims). Optimisé **14,5 Mo → 1,25 Mo** (`gltf-transform resize 1024` puis `webp` — PAS `optimize`/simplify, casserait le skin). **5 anims** : `Lower_Weapon_Look_Raise` (=idle), `Walking`, `Running`, `Jump_Over_Obstacle_2` + `Back_Jump` (squelette 26 os, 10 355 tris). Source brute `models/character_src.glb` (gitignore).
- ⚙️ Dans `index.html` : `GLTFLoader` → auto-scale **HERO_HEIGHT=0.7** + pieds au sol + centrage + `MODEL_YAW=0` (dos visible en marche avant). **Matériau forcé `DoubleSide`+opaque** (normales Meshy inversées → sinon transparent de dos). Machine à états `AnimationMixer` : fondus **idle/walk/jump** (poids `heroIdleW/WalkW/JumpW`), jump relancé au décollage. `runAction` dispo (sprint futur).
