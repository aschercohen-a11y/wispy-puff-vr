...
- 🍄 **Champignons = obstacles-plateformes** (`mushroomObstacles[]` x,z,r,top depuis bbox) : (1) au sol on est **bloqué** (push-out, pas de chute) ; (2) on peut **sauter par-dessus** ; (3) on peut **atterrir/se tenir dessus** (landing top si `heroVelY<=0 && prevY>=top`) ; (4) si on **clippe le côté EN L'AIR** (saut raté) → **trébuche** : `heroFallT=1.4s`, snap au sol, élan `heroFallVX/VZ` = facing×2.6 (tombe « de l'autre côté »), anim `falling_down`.
