...
- 🥽 **VR = 3e personne qui suit** (choix user) : `updateFollowRigVR()` place le rig à `VR_DIST=5` derrière le perso selon `camYaw` ; stick gauche = marche (relative caméra), stick droit = tourne `camYaw`, A/B = saut. PC : `updateFollowCam()` (souris = orbite). **À FAIRE : brancher Running sur sprint ; vérifier root motion (glissement) ; régler VR_DIST si besoin.**
