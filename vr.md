...
**☀️ Éclairage activable (fait dans la foulée).** La netteté rétablie, il restait l'effet carton-pâte : en unlit une face au soleil et une face à l'ombre ont la même couleur, et en VR la stéréo attend du relief. Bouton **☀️ Éclairage** (barre d'outils, à côté de Collines) — **bascule, pas remplacement**, car l'unlit garantit des couleurs identiques entre tuiles (c'est ce qui avait réglé les raccords trottoir/route). `MeshLambertMaterial` (pas Standard : le PBR coûte cher au pixel sur Quest pour rien ici). Jumeaux plat/éclairé en `WeakMap` → la bascule ne recrée rien. **Réglages tirés des captures :** ambiante forte (0.85) + soleil modéré (0.45) et **rasant** `(0.62, 0.5, 0.34)` pour accrocher les façades, pas les toits (un premier essai 0.5/0.8 assombrissait tout) ; et **seul `worldGroup` est éclairé** — ombrer `bgGroup` noircissait les montagnes de fond, qui sont des silhouettes calibrées pour le brouillard. `refresh()` réapplique le mode (les objets posés sont clonés depuis des matériaux plats). Actif par défaut, choix en localStorage. Test : `dev/shot_lighting.js`, **sans aucun clic souris**.
...
- 🔄 **VIRAGE GAMEPLAY** : fini le fantôme volant → **personnage jouable 3e personne** (marche + saut + gravité + ramasse les diamants). Contrôles : PC joystick+WASD marche / souris = orbite caméra / Espace = saut ; mobile joystick+bouton saut ; VR stick = marche, A/B = saut.
- ✅ **VRAI PERSO Meshy intégré** : `models/character.glb` (bandicoot style Crash). Optimisé **14,6 Mo → 1,3 Mo** (`gltf-transform resize 1024` puis `webp` — PAS `optimize`/simplify, casserait le skin). **6 anims** : `Lower_Weapon_Look_Raise` (=idle), `Walking`, `Running`, `Jump_Over_Obstacle_2`, `Back_Jump`, `falling_down` (squelette 26 os, 10 355 tris). Source brute `models/character_src.glb` (gitignore).
- ⚙️ Dans `index.html` : `GLTFLoader` → auto-scale **HERO_HEIGHT=0.55** + pieds au sol + centrage + `MODEL_YAW=0` (dos visible en marche avant). **Matériau `DoubleSide`+opaque** (normales Meshy inversées → sinon transparent de dos). Machine à états `AnimationMixer` : fondus **fall > jump > walk > idle** (poids `heroFallW/JumpW/WalkW/IdleW`).
- 🍄 **Champignons = beau modèle Meshy** (`models/mushroom_meshy.glb`, 23.7Mo→145Ko, centré donc grounding via `capOffset*scale`, DoubleSide, statiques — pas de dandinement). Source `mush_src.glb` (gitignore). Anciens `mushroom.glb` cartoon plus utilisés.
- 🍄 **Obstacles-plateformes** (`mushroomObstacles[]` {x,z,r,top,mesh,base,gk,squash,squashV}) : (1) au sol **bloqué** (push-out, pas de chute) ; (2) **sauter par-dessus** ; (3) **se tenir dessus** (landing si `heroVelY<=0 && prevY>=top`, recentrage 0.2 car perso + large) ; (4) **clippe le côté EN L'AIR** → **trébuche** (`heroFallT=1.4`, élan facing×2.6, anim `falling_down`). **RESSORT** : atterrissage franc (`heroVelY<-1.5`) → `squashV=-7`, ressort amorti (chapeau s'enfonce+rebondit, `mesh.position.y=gk*base*fy` garde la base au sol).
- ⚠️ **ROOT MOTION par clip** (valeurs Meshy en **cm**, Hips repos Y≈69.8) : on retire `Hips.position` des anims normales (sur place), MAIS pour `falling_down` on **garde la composante Y** (le corps descend au sol, sinon il flotte) en figeant juste X/Z au repos. NE JAMAIS retirer les `.position` des autres os (tête se déforme).
- 🥽 **VR = 3e personne qui suit** (choix user) : `updateFollowRigVR()` place le rig à `VR_DIST=5` derrière le perso selon `camYaw` ; stick gauche = marche (relative caméra), stick droit = tourne `camYaw`, A/B = saut. PC : `updateFollowCam()` (souris = orbite). Perso **réduit à 60% en VR** (`VR_HERO_SCALE=0.6`, paraissait géant sinon).
- ⚠️ **ROOT MOTION — piège** : le saut/marche Meshy translate le bassin → le modèle partait en avant puis re-snappait. Fix = retirer les pistes `.position` **UNIQUEMENT de l'os `Hips`** au chargement (`clips.forEach`... filtre `Hips.position`). NE PAS retirer toutes les `.position` : la tête (`Head`) a une position de repos ≠ animée → elle se déforme quand elle tourne (bug idle observé). La physique gère saut/déplacement.
- 💡 **À FAIRE** : brancher Running sur un sprint ; régler `VR_HERO_SCALE`/`VR_DIST` selon ressenti casque.
