...
- ✅ **VRAI PERSO Meshy intégré** : `models/character.glb` (bandicoot style Crash, généré sur Meshy image-to-3D + rig + anims). Optimisé **14,4 Mo → 1,1 Mo** (webp+resize 1024, `gltf-transform resize` puis `webp` — PAS `optimize`/simplify pour ne pas casser le skin). Contient **2 anims : Walking + Running** (+ squelette 26 os, 10 355 tris). Dans `index.html` : `GLTFLoader` → auto-scale (HERO_HEIGHT=1.5) + pieds au sol + `MODEL_YAW=0` (validé : dos visible en marche avant) ; `AnimationMixer` joue Walking (`timeScale` 1.3 en mouvement, 0 à l'arrêt). Source brute = `models/character_src.glb` (gitignore). **À FAIRE : idle + jump (pas dans le GLB) ; brancher Running sur un sprint ; vérifier pas de glissement (root motion).**
